...
namespace mmh {
    ...
    UIntegerNaturalFormatter::UIntegerNaturalFormatter(t_uint64 p_val, unsigned p_width, unsigned p_base)
        : m_value(p_val)
    {
    ...
    FileSizeFormatter::FileSizeFormatter(t_uint64 size)
    {
        ...
        if (b_minor) {
            ...
            if (i > minor_digits && ((remainder % power_of_ten(i - minor_digits)) / power_of_ten(i - minor_digits - 1))
                >= 5) {
        ...
        *this << major;
        if (b_minor) {
            ...
            *this << pfc::stringcvt::string_utf8_from_wide(separator, tabsize(separator)) << pfc::format_uint(
                minor, minor_digits);
        }
